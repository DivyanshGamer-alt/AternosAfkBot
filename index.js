...
function startBot(version = false) {
    ...
    bot.on('end', () => {
        console.log(`❌ Bot disconnected (version: ${version || "auto"}). Retrying...`)
        setTimeout(() => tryNextVersion(version), 5000)
    })
...
function attachEvents() {
    bot.on('login', function () {
        ...
        if (data["login-enabled"] == "true") {
            setTimeout(() => bot.chat(data["login-cmd"]), 1000)
            setTimeout(() => bot.chat(data["register-cmd"]), 3000);
        }
        for (let i = 0; i < 10; i++) {
            task(i);
        }
...
app.get('/', (req, res) => {
    ...
    <br><br>
    Die Counter <b>${death}</b><br><br>
    Simp Counter <b>${simp}</b><br><br>
    Popularity Counter <b>${popularity}</b><br><br>
